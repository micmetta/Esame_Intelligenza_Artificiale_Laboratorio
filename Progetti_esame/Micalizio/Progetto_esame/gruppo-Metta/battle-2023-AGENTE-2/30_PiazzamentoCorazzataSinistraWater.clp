...
(defmodule PIAZZAMENTO_CORAZZATA_SINISTRA_WATER (import MAIN ?ALL) (import ENV ?ALL) (import AGENT ?ALL) (import GESTORE_SINISTRA_WATER ?ALL) (export ?ALL))
...
;; 1-in orizzontale (ovviamente VERSO sinistra, supponendo che la cella_max sia il right)
;; 2-in verticale (assumendo che la cella_max sia il bot) 
;; 3-in verticale (assumendo che la cella_max sia il top)
...
(deftemplate celle_adiacenti_a_cella_max  
    ...
    (slot x_row_sotto_3) ; riga +2 sotto cella_max
    ...
    (slot y_col_sinistra_3) ; colonna +3 a sinistra della cella_max
)
...
(deftemplate piazzamento_corazzata_verticale_cella_max_bot
    (slot x)
    (slot y)
)
(deftemplate piazzamento_corazzata_verticale_cella_max_top
    (slot x)
    (slot y)
)
(deftemplate piazzamento_corazzata_verticale ;; in caso di conflitti sulla dir verticale, conterrà comunque il punto di partenda della corazzata che ha vinto lungo la dir verticale
    (slot x)
    (slot y)
)
...
=>
    (assert (celle_adiacenti_a_cella_max (x_row_sopra_1 (- ?x 1)) (x_row_sopra_2 (- ?x 2)) (x_row_sopra_3 (- ?x 3))
                                         (x_row_sotto_1 (+ ?x 1)) (x_row_sotto_2 (+ ?x 2)) (x_row_sotto_3 (+ ?x 3)) 
...
(defrule posizionamento_corazzata_in_orizzontale_supponendo_cella_max_right (declare (salience 26))
    ...
    (celle_adiacenti_a_cella_max (x_row_sopra_1 ?x_row_sopra_1) (x_row_sopra_2 ?x_row_sopra_2) (x_row_sopra_3 ?x_row_sopra_3)
                                 (x_row_sotto_1 ?x_row_sotto_1) (x_row_sotto_2 ?x_row_sotto_2) (x_row_sotto_3 ?x_row_sotto_3) 
...
=>
    ...
    (assert (piazzamento_corazzata_orizzontale (x ?x) (y ?y_col_sinistra_3))) ; per semplicità qui setto solamente la cella da dove partirà la corazzata (da sinistra)
)
...
=>
    ...
    ; e quindi faccio questa assert:
    (assert (piazzamento_corazzata_verticale_cella_max_bot (x ?x) (y ?y))) ; per semplicità qui setto solamente la cella da dove partirà la corazzata (dal basso)
)
...
(defrule posizionamento_corazzata_in_verticale_supponendo_cella_max_top (declare (salience 26))
    ...
    ; 5) E inoltre mi devo assicurare che anche tre righe subito sotto possa mettere 
    ;    la bandierina che supporremo corrisponda al “bot” e quindi deve essere vero che:
    (k-per-row (row ?x_row_sotto_3) (num ?max_pezzi_row_sotto_3))
    (k-per-row-bandierine-posizionate (row ?x_row_sotto_3) (num ?num_b_row_sotto_3))
    (test (>= (- ?max_pezzi_row_sotto_3 ?num_b_row_sotto_3) 1))
...
=>
    ...
    ; A QUESTO PUNTO SONO CERTO DI POTER PIAZZARE UNA CORAZZATA nelle seguenti posizioni
    ; (x,y)(dove sappiamo esserci cella_max) - (x+1,y) (primo middle) – (x+2,y)(altro middle) – (x+3,y)(bot) 
    ; e quindi faccio questa assert:
    (assert (piazzamento_corazzata_verticale_cella_max_top (x ?x_row_sotto_3) (y ?y))) ; per semplicità qui setto solamente la cella da dove partirà la corazzata (dal basso)
)
...
;; REGOLE CHE SARANNO ESEGUITE SOLAMENTE SE C'E' UN POSSIBILE PIAZZAMENTO VERSO UNA SOLA DELLE DUE DIREZIONI VERTICALI:
...
(defrule aggiunta_fatti_per_gestire_solo_il_caso_verticale_cella_max_bot (declare (salience 25))
    ...
    (piazzamento_corazzata_verticale_cella_max_bot (x ?x_row_bot_caso1) (y ?y_caso1))
    (not (piazzamento_corazzata_verticale_cella_max_top (x ?x_row_bot_caso2) (y ?y_caso2)))

=>  
    (assert (piazzamento_corazzata_verticale(x ?x_row_bot_caso1) (y ?y_caso1)))
)
(defrule aggiunta_fatti_per_gestire_solo_il_caso_verticale_cella_max_top (declare (salience 25))

    (piazzamento_corazzata_verticale_cella_max_top (x ?x_row_bot_caso2) (y ?y_caso2))
    (not (piazzamento_corazzata_verticale_cella_max_bot (x ?x_row_bot_caso1) (y ?y_caso1)))

=>  
    (assert (piazzamento_corazzata_verticale(x ?x_row_bot_caso2) (y ?y_caso2)))
)
...
(defrule aggiunta_fatti_per_gestione_conflitti_solo_verticale (declare (salience 25))
    ...
    (piazzamento_corazzata_verticale_cella_max_bot (x ?row_bot_caso1) (y ?y))
    (piazzamento_corazzata_verticale_cella_max_top (x ?row_bot_caso2) (y ?y))
...
(defrule vince_verticale_caso1 (declare (salience 23))
    ...
    ; 1) devono essere veri questi fatti in WM
    ?piazzamento_caso1 <- (piazzamento_corazzata_verticale_cella_max_bot (x ?row_bot_caso1) (y ?y))
    ?piazzamento_caso2 <- (piazzamento_corazzata_verticale_cella_max_top (x ?row_bot_caso2) (y ?y))
...
(defrule vince_verticale_supponendo_cella_max_bot (declare (salience 20))
    ...
    (piazzamento_corazzata_verticale_cella_max_bot (x ?x) (y ?y)) ; punto di mutua esclusione
...
(defrule vince_verticale_supponendo_cella_max_top (declare (salience 20))
    ...
    (piazzamento_corazzata_verticale_cella_max_top (x ?x) (y ?y)) ; punto di mutua esclusione
...
=>
    ...
    (assert (k_cell_agent (x (- ?row_bot 1)) (y ?y) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che ho posizionato una bandierina in cella(x-1,y)
    (assert (k_cell_agent (x (- ?row_bot 2)) (y ?y) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che ho posizionato una bandierina in cella(x-2,y)
    (assert (k_cell_agent (x (- ?row_bot 3)) (y ?y) (content sconosciuto) (considerato false) (current yes))) ; questa è la cella_max nella quale sto assumendo che ci sia il top in cella(x-3,y)
...
(defrule vince_orizzontale_senza_conflitto (declare (salience 10)) ; è mutuamente esclusiva con quella di sotto
    ...
    (not (piazzamento_corazzata_verticale (x ?x_row_bot) (y ?y))) ; aggiunto
    ?f_piazzamento_orizzontale <- (piazzamento_corazzata_orizzontale (x ?x) (y ?col_left))
...
(defrule vince_verticale_supponendo_cella_max_bot_senza_conflitto (declare (salience 10))
    ...
    ?f_piazzamento_verticale <- (piazzamento_corazzata_verticale_cella_max_bot (x ?row_bot) (y ?y))
...
(defrule vince_verticale_supponendo_cella_max_top_senza_conflitto (declare (salience 10))
    ...
    (not (piazzamento_corazzata_orizzontale (x ?x) (y ?y_col_left))) ; aggiunto
    ?f_piazzamento_verticale <- (piazzamento_corazzata_verticale_cella_max_top (x ?row_bot) (y ?y))
