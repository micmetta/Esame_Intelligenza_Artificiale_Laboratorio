...
(defmodule PIAZZAMENTO_INCROCIATORE_SU_WATER (import MAIN ?ALL) (import ENV ?ALL) (import AGENT ?ALL) (import GESTORE_SU_WATER ?ALL) (export ?ALL))



;; Sappiamo che nella cella subito sotto rispetto alla cella_max c'è water e quindi l'unico modo per cercare
...
;; 1-in verticale (ovviamente VERSO l'alto, supponendo che la cella_max sia il bot)
...
;; mi serve per conoscere le coordinate di tutte le celle nelle quali posso provare a piazzare qualcosa partendo da cella_max(x,y)
(deftemplate celle_adiacenti_a_cella_max 
    (slot x_row_sopra_1) ; riga subito sopra a cella_max
    (slot x_row_sopra_2) ; riga -2 sopra al cella_max
...
=>
    (assert (celle_adiacenti_a_cella_max (x_row_sopra_1 (- ?x 1)) (x_row_sopra_2 (- ?x 2))
                                         (y_col_sinistra_1 (- ?y 1)) (y_col_sinistra_2 (- ?y 2))
                                         (y_col_destra_1 (+ ?y 1)) (y_col_destra_2 (+ ?y 2))))
)
;(defrule stampa (declare (salience 50))
;    (celle_adiacenti_a_cella_max (x_row_sopra_1 ?x_row_sopra_1) (x_row_sopra_2 ?x_row_sopra_2)
...
(defrule posizionamento_incrociatore_in_verticale_supponendo_cella_max_bot (declare (salience 26))
    ...
    ;    la bandierina che supporremo corrisponda al "middle" partendo dal basso e quindi deve essere vero che:
    ...
    ; 4) E inoltre mi devo assicurare che due righe subito sopra a cella_max possa mettere 
    ;    la bandierina che supporremo corrisponda al “top” partendo dal basso e quindi deve essere vero che:
    (k-per-row (row ?x_row_sopra_2) (num ?max_pezzi_row_sopra_2))
    (k-per-row-bandierine-posizionate (row ?x_row_sopra_2) (num ?num_b_row_sopra_2))
    ...
    (not (k-cell (x ?x_row_sopra_2) (y ?y) (content water)))
    ...
    (not (k_cell_agent (x ?x_row_sopra_2) (y ?y) (content ?content) (considerato true) (current no)))
...
=>
    ...
    ; (x,y)(dove sappiamo esserci cella_max) - (x-1,y) (middle) – (x-2,y)(top)
    ; e quindi faccio questa assert:
    (assert (piazzamento_incrociatore_verticale (x ?x) (y ?y))) ; per semplicità qui setto solamente la cella da dove partirà la incrociatore (dal basso)
)
...
(defrule posizionamento_incrociatore_in_orizzontale_supponendo_cella_max_right (declare (salience 26))
    ...
    ;    la bandierina che supporremo corrisponda al “left” partendo da destra e quindi deve essere vero che:
...
=>
    ...
    ; A QUESTO PUNTO SONO CERTO DI POTER PIAZZARE UNA incrociatore nelle seguenti posizioni
    ; (x,y-2) (left) – (x,y-1)(middle) – (x,y)(dove sappiamo esserci cella_max) 
    ; e quindi faccio questa assert:
...
=>
    ...
    ; A QUESTO PUNTO SONO CERTO DI POTER PIAZZARE UNA incrociatore nelle seguenti posizioni
    ; (x,y)(dove sappiamo esserci cella_max) - (x,y+1) (middle) – (x,y+2)(right) 
    ; e quindi faccio questa assert:
...
; -	Le regole di sotto gestiranno UN CONFLITTO CHE SI VERIFICA QUANDO POSSIAMO PIAZZARE UN incrociatore 
...
=>
	...
	; posso aggiornare la nostra struttura dove manteniamo aggiornate le posizioni nelle quali
	; l'agente posiziona le sue bandierine:
	(assert (k_cell_agent (x ?x) (y ?col_left) (content sconosciuto) (considerato false) (current yes))) ; questa è la cella_max nella quale sto assumendo che ci sia il right in cella(x,y)
...
=>
	...
	; posso aggiornare la nostra struttura dove manteniamo aggiornate le posizioni nelle quali
	; l'agente posiziona le sue bandierine:
	(assert (k_cell_agent (x ?x) (y ?col_left) (content sconosciuto) (considerato false) (current yes))) ; questa è la cella_max nella quale sto assumendo che ci sia il right in cella(x,y-3)
...
(defrule cancello_celle_adiacenti_a_cella_max_ormai_inutili(declare (salience 1))
    ...
    ?cella_da_cancellare <- (celle_adiacenti_a_cella_max (x_row_sopra_1 ?x_row_sopra_1) (x_row_sopra_2 ?x_row_sopra_2)
                                                         (y_col_sinistra_1 ?y_col_sinistra_1) (y_col_sinistra_2 ?y_col_sinistra_2)
                                                         (y_col_destra_1 ?y_col_destra_1) (y_col_destra_2 ?y_col_destra_2))
=>
