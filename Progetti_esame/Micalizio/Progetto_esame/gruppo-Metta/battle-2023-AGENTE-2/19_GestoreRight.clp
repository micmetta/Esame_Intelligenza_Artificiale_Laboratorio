...
(defmodule GESTORE_RIGHT (import MAIN ?ALL) (import ENV ?ALL) (import AGENT ?ALL) (export ?ALL))

; Questo modulo si preoccuperà di chiamare prima il modulo "PIAZZAMENTO_CORAZZATA" e se questo non riesce in nessun
; modo a piazzare una corazzata (conoscendo che in cella(x,y) c'è un right) allora richiamerà il modulo
...
(deftemplate celle_sinistra_a_right_in_gestore_right
    (slot y_col_sinistra_1) ; riga subito a sinistra rispetto al right
    (slot y_col_sinistra_2) ; riga -2 a sinistra rispetto al right
    (slot y_col_sinistra_3) ; riga -3 a sinistra rispetto al right
)
...
; sono riusciti a piazzare una nave perchè con la regola di sotto il modulo GESTORE_RIGHT
...
(defrule controllo_piazzamento_nave_gestore_right (declare (salience 170))
...
;; regola qui sotto per ritrattare la cella_right ormai risolta:
(defrule cancellazione_cella_right_risolta (declare (salience 169))
    
    (nave_piazzata_gestore (piazzamento true)) ; controllo se sono riuscito a piazzare una nave
    
    ; Aggancio la cella_right_corrente che il modulo AGENT mi ha detto di considerare
    ; e per la quale l'agente è riscito a piazzare una nave:
    ?cella_right_corrente <- (cella_right (x ?x) (y ?y) (considerata true))

    ; aggiunta..
    ?celle_sinistra_a_right_in_gestore_right <- (celle_sinistra_a_right_in_gestore_right (y_col_sinistra_1 ?y_col_sinistra_1) (y_col_sinistra_2 ?y_col_sinistra_2) (y_col_sinistra_3 ?y_col_sinistra_3))

=>
    (retract ?cella_right_corrente) ;; ormai l'ho completata quindi la tolgo in modo tale che l'agente non la rivaluti

    ; aggiunta..
    (retract ?celle_sinistra_a_right_in_gestore_right) ;; ormai non mi serve più
...
;;(defrule controllo_nessun_piazzamento_nave_gestore_right (declare (salience 168))
...
;;    (nessuna_nave_posizionata (posizionamento nessuno))
    ...
    ; Aggancio la cella_right che il modulo AGENT mi ha detto di considerare
    ; e per la quale l'agente è riscito a piazzare una nave
;;    ?cella_right_corrente <- (cella_right (x ?x) (y ?y) (considerata false))
;;    ?new_cella <- (k_cell_agent (x ?x) (y ?y) (content right) (considerato false) (current yes))

;;=>
;;    (retract ?cella_right_corrente) ;; ormai ci ho provato quindi la tolgo in modo tale che l'agente non la rivaluti
...
(defrule controllo_piazzamento_cacciatorpediniere (declare (salience 150))
    ...
    (cella_right (x ?x) (y ?y) (considerata true)) ; controllo che ci sia una cella_right che il modulo AGENT mi ha detto di considerare
...
=>  
    ...
    (focus PIAZZAMENTO_CACCIATORPEDINIERE_RIGHT)
)
...
=>  
    ...
    ; richiamo il sotto-modulo che si preoccuperà di posizionare l'incrociatore
    (focus PIAZZAMENTO_INCROCIATORE_RIGHT)
)
...
=>  
    ...
    (focus PIAZZAMENTO_CORAZZATA_RIGHT)
)
...
(defrule controllo_se_non_sono_riuscito_a_piazzare_nessuna_nave_precedente (declare (salience 100))
    ...
    ; controllo che ci sia una cella_right che il modulo AGENT mi ha detto di considerare:
    ?cella_right_corrente <- (cella_right (x ?x) (y ?y) (considerata true))
...
=>
    ...
    ;; partendo da una cella nella quale è certo che ci sia il "right" di una nave.
...
;; la colonna "x" rimane fissa e controllo se lungo questa riga devo fare l'unguess (perchè stiamo trattando un right)
(defrule eseguo_unguess_lungo_la_stessa_riga_di_cella_right (declare (salience 99))

    ?cella_right_corrente <- (cella_right (x ?x) (y ?y) (considerata true)) ; prendo le info della cella che mi ha 
    ...
    ; verifico di NON riuscire a piazzare una bandierina nella riga in cui si trova la cella right:
...
=>
    ...
    ; asserisco la cella da cancellare (in questo caso sarà quella presente nella riga subito sopra alla cella_right
...
;; la riga "x" adesso può variare rispetto a quella di cella_right e considero come colonna "y" 
;; quella subito a sinistra rispetto a dove si trova cella_right (perchè stiamo trattando un right) per cercare di capire se in questa
;; colonna "y_col_sinistra_1" devo fare l'unguess:
(defrule eseguo_unguess_su_colonna_sinistra_rispetto_a_cella_right (declare (salience 99))
    ...
    ; verifico di riuscire a piazzare una bandierina nella riga in cui si trova la cella right:
    ...
    ; verifico di non riuscire a piazzare una bandierina nella colonna subito a destra rispetto alla cella right:
    ...
    (k-per-col (col ?y_col_sinistra_1) (num ?max_pezzi_col_destra_1))
    (k-per-col-bandierine-posizionate (col ?y_col_sinistra_1) (num ?num_b_col_destra_1))
    (test (< (- ?max_pezzi_col_destra_1 ?num_b_col_destra_1) 1))

    ; mi assicuro che sulla cella sulla quale farò l'unguess l'agente abbia già posizionato una bandierina:
    ; (aggiunta)
    (exec (step ?s) (action guess) (x ?x_var) (y ?y_col_sinistra_1))

    ; A questo punto sono certo che devo togliere una bandierina dalla "y_col_sinistra_1" dove però
    ; la riga non dovrà essere uguale ad "x" e quindi
    ; prendo la posizione di una delle bandierine che l'agente ha posizionato lungo
    ; la colonna "y_col_sinistra_1" (la "x" invece sarà diversa da quella della cella right)
    ?k_cell_agent <- (k_cell_agent (x ?x_var) (y ?y_col_sinistra_1) (content ?val_content) (considerato ?val_considerato) (current ?val_current))
    ...
    ; mi assicuro che la bandierina che andrò a togliere non faccia riferimento ad un fatto iniziale che 
    ; mi assicura che in quella posizione ci sia un qualche pezzo di una nave:
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_1) (content top)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_1) (content bot)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_1) (content left)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_1) (content right)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_1) (content middle)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_1) (content sub)))


    ?posizione_bandierina_da_togliere <- (k-per-row-bandierine-posizionate (row ?x_var) (num ?n1)) 
    ?posizione_colonna_bandierina_da_togliere <- (k-per-col-bandierine-posizionate (col ?y_col_sinistra_1) (num ?n2)) 

=>

    ; asserisco la cella da cancellare (in questo caso sarà quella presente nella colonna subito a destra rispetto alla cella right
    ; per questo motivo c'è "?y_col_sinistra_1"):
    (assert (bandierina_da_cancellare (x ?x_var) (y ?y_col_sinistra_1) (content sconosciuto) (considerato true) (current yes)))
...
; la riga "x" varia rispetto a quella di cella_right e la colonna che consideriamo adesso è la "y_col_sinistra_2" perchè stiamo trattando un right:
(defrule eseguo_unguess_su_due_colonne_a_sinistra_rispetto_a_cella_right (declare (salience 99))
    ...
    ; (questo controllo mi permette di sapere con certezza che la bandierina da togliere deve essere una
    ;  di quelle presente proprio in questa colonna!)
    (k-per-col (col ?y_col_sinistra_2) (num ?max_pezzi_col_destra_1))
    (k-per-col-bandierine-posizionate (col ?y_col_sinistra_2) (num ?num_b_col_destra_1))
    (test (< (- ?max_pezzi_col_destra_1 ?num_b_col_destra_1) 1))

    ; mi assicuro che sulla cella sulla quale farò l'unguess l'agente abbia già posizionato una bandierina:
    ; (aggiunta)
    (exec (step ?s) (action guess) (x ?x_var) (y ?y_col_sinistra_2))


    ; A questo punto sono certo che devo togliere una bandierina dalla "y_col_sinistra_2" dove però
    ; la riga non dovrà essere uguale ad "x" e quindi
    ; prendo la posizione di una delle bandierine che l'agente ha posizionato lungo
    ; la colonna "y_col_sinistra_2" (la "x" invece sarà diversa da quella della cella right)
    ?k_cell_agent <- (k_cell_agent (x ?x_var) (y ?y_col_sinistra_2) (content ?val_content) (considerato ?val_considerato) (current ?val_current))
    ...
    ; mi assicuro che la bandierina che andrò a togliere non faccia riferimento ad un fatto iniziale che 
    ; mi assicura che in quella posizione ci sia un qualche pezzo di una nave:
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_2) (content top)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_2) (content bot)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_2) (content left)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_2) (content right)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_2) (content middle)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_2) (content sub)))

    ?posizione_bandierina_da_togliere <- (k-per-row-bandierine-posizionate (row ?x_var) (num ?n1)) 
    ?posizione_colonna_bandierina_da_togliere <- (k-per-col-bandierine-posizionate (col ?y_col_sinistra_2) (num ?n2)) 

=>

    ; asserisco la cella da cancellare (in questo caso sarà quella presente nella colonna subito a destra rispetto alla cella right
    ; per questo motivo c'è "?y_col_sinistra_2"):
    (assert (bandierina_da_cancellare (x ?x_var) (y ?y_col_sinistra_2) (content sconosciuto) (considerato true) (current yes)))
...
; la riga "x" varia rispetto a quella di cella_right e la colonna che consideriamo adesso è la "y_col_sinistra_3" perchè stiamo trattando un right:
(defrule eseguo_unguess_su_tre_colonne_a_sinistra_rispetto_a_cella_right (declare (salience 99))
    ...
    ; (questo controllo mi permette di sapere con certezza che la bandierina da togliere deve essere una
    ;  di quelle presente proprio in questa colonna!)
    (k-per-col (col ?y_col_sinistra_3) (num ?max_pezzi_col_destra_1))
    (k-per-col-bandierine-posizionate (col ?y_col_sinistra_3) (num ?num_b_col_destra_1))
    (test (< (- ?max_pezzi_col_destra_1 ?num_b_col_destra_1) 1))


    ; mi assicuro che sulla cella sulla quale farò l'unguess l'agente abbia già posizionato una bandierina:
    ; (aggiunta)
    (exec (step ?s) (action guess) (x ?x_var) (y ?y_col_sinistra_3))


    ; A questo punto sono certo che devo togliere una bandierina dalla "y_col_sinistra_3" dove però
    ; la riga non dovrà essere uguale ad "x" e quindi
    ; prendo la posizione di una delle bandierine che l'agente ha posizionato lungo
    ; la colonna "y_col_sinistra_3" (la "x" invece sarà diversa da quella della cella right)
    ?k_cell_agent <- (k_cell_agent (x ?x_var) (y ?y_col_sinistra_3) (content ?val_content) (considerato ?val_considerato) (current ?val_current))
    ...
    ; mi assicuro che la bandierina che andrò a togliere non faccia riferimento ad un fatto iniziale che 
    ; mi assicura che in quella posizione ci sia un qualche pezzo di una nave:
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_3) (content top)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_3) (content bot)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_3) (content left)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_3) (content right)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_3) (content middle)))
    (not (k-cell (x ?x_var) (y ?y_col_sinistra_3) (content sub)))

    ?posizione_bandierina_da_togliere <- (k-per-row-bandierine-posizionate (row ?x_var) (num ?n1)) 
    ?posizione_colonna_bandierina_da_togliere <- (k-per-col-bandierine-posizionate (col ?y_col_sinistra_3) (num ?n2)) 

=>

    ; asserisco la cella da cancellare (in questo caso sarà quella presente nella colonna subito a destra rispetto alla cella right
    ; per questo motivo c'è "?y_col_sinistra_3"):
    (assert (bandierina_da_cancellare (x ?x_var) (y ?y_col_sinistra_3) (content sconosciuto) (considerato true) (current yes)))
...
(defrule controllo_incremento_corazzate_da_trovare (declare (salience 98))
    ...
    ; per la stessa cella right di partenza
...
=>
    ...
    (retract ?cella_right) ;; non serve più
    (retract ?fai_unguess)
)