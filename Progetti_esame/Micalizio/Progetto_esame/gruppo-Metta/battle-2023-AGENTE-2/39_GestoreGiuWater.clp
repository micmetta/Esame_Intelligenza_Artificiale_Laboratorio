...
(defmodule GESTORE_GIU_WATER (import MAIN ?ALL) (import ENV ?ALL) (import AGENT ?ALL) (import GESTORE_WATER ?ALL) (export ?ALL))

; Questo modulo SA' CHE SOTTO LA (cella_max (x) (y) (direzione giu)) c'è WATER 
; e quindi anche tutti i suoi sotto-moduli di piazzamento sapranno questa cosa e quindi questi ultimi
; cercheranno di piazzare una qualche nave partendo proprio (cella_max (x) (y) (direzione giu))
; SFRUTTANDO L'INFORMAZIONE CHE NELLA CELLA SUBITO SOTTO della cella_max C'E' WATER.
...
(deffacts fatti_iniziali_giu_water
...
; sono riusciti a piazzare una nave perchè con la regola di sotto il modulo GESTORE_GIU_WATER
...
(defrule controllo_piazzamento_nave_gestore_water_giu (declare (salience 170))
...
(defrule controllo_nessun_piazzamento_nave_gestore_water_giu (declare (salience 168))
...
(defrule controllo_piazzamento_sottomarino (declare (salience 150))
    ...
    (cella_max (x ?x) (y ?y) (direzione giu)) ; controllo che ci sia una cella_middle che il modulo GESTORE_WATER mi ha detto di considerare
...
=>  
    ...
    (focus PIAZZAMENTO_SOTTOMARINO_GIU_WATER)
)
...
=>  
    ...
    ; richiamo il sotto-modulo che si preoccuperà di posizionare l'incrociatore
    (focus PIAZZAMENTO_CACCIATORPEDINIERE_GIU_WATER)
)
...
=>  
    ...
    ; richiamo il sotto-modulo che si preoccuperà di posizionare l'incrociatore
    (focus PIAZZAMENTO_INCROCIATORE_GIU_WATER)
)
...
=>  
    ...
    (focus PIAZZAMENTO_CORAZZATA_GIU_WATER)
)
...
(defrule controllo_se_non_sono_riuscito_a_piazzare_nessuna_nave_precedente (declare (salience 100))
    ...
    ?cella_max_corrente <- (cella_max (x ?x) (y ?y) (direzione giu))
...
=>
    ...
    ;; nave è stata posizionata in modo tale che la regola "controllo_nessun_piazzamento_nave_gestore_water_giu"
