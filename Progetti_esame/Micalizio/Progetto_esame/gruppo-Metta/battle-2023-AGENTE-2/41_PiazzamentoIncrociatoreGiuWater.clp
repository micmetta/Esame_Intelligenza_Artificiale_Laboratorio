...
(defmodule PIAZZAMENTO_INCROCIATORE_GIU_WATER (import MAIN ?ALL) (import ENV ?ALL) (import AGENT ?ALL) (import GESTORE_GIU_WATER ?ALL) (export ?ALL))



;; Sappiamo che nella cella subito sopra rispetto alla cella_max c'è water e quindi l'unico modo per cercare
;; di piazzare un incrociatore partendo dalla cella_max (DOVE SONO CERTO DI POTER POSIZIONARE UNA BANDIERINA ma NON SO NULLA SU cosa ci sia al suo 
;; interno) è quella di metterla in una delle seguenti posizioni:

;; 1-in verticale (ovviamente VERSO il basso, supponendo che la cella_max sia il top)
;; 2-in orizzontale (assumendo che la cella_max sia il left)
;; 3-in orizzontale (assumendo che la cella_max sia il right)
...
(deftemplate celle_adiacenti_a_cella_max 
    (slot x_row_sotto_1) ; riga subito sotto a cella_max
    (slot x_row_sotto_2) ; riga +2 sotto al cella_max

    (slot y_col_sinistra_1) ; colonna subito a sinistra della cella_max
    (slot y_col_sinistra_2) ; colonna +2 a su della cella_max

    (slot y_col_destra_1) ; colonna subito a sinistra della cella_max
    (slot y_col_destra_2) ; colonna +2 a su della cella_max
)
...
(deftemplate celle_incrociatore_caso1_orizzontale
    (slot y_col_left) 
    (slot y_col_middle_1) 
    (slot y_col_right)
)
(deftemplate celle_incrociatore_caso2_orizzontale
    (slot y_col_left) 
    (slot y_col_middle_1) 
    (slot y_col_right)
)
...
(deftemplate piazzamento_incrociatore_orizzontale_cella_max_left
    (slot x)
    (slot y)
)
(deftemplate piazzamento_incrociatore_orizzontale_cella_max_right
    (slot x)
    (slot y)
)
(deftemplate piazzamento_incrociatore_orizzontale ;; in caso di conflitti sulla dir orizzontale, conterrà comunque il punto di partenda della incrociatore che ha vinto lungo la dir orizzontale
    (slot x)
    (slot y)
)


(deftemplate scores_conflitto_orizzontale ; mi permette di calcolare lo score per le due direzioni orizzontale qualora ci fossero conflitti
...
(defrule aggiunta_fatti_per_reg_successive (declare (salience 50))
    (cella_max (x ?x) (y ?y) (direzione giu))
=>
    (assert (celle_adiacenti_a_cella_max (x_row_sotto_1 (+ ?x 1)) (x_row_sotto_2 (+ ?x 2))
                                         (y_col_sinistra_1 (- ?y 1)) (y_col_sinistra_2 (- ?y 2))
                                         (y_col_destra_1 (+ ?y 1)) (y_col_destra_2 (+ ?y 2))))
)


;; DA QUI PARTE IL CONTROLLO SULLA DIREZIONE VERTICALE.

(defrule posizionamento_incrociatore_in_verticale_supponendo_cella_max_top (declare (salience 26))
    ...
    (test (>= (- ?max_pezzi_col ?num_b_col) 3)) ; controllo se la differenza è maggiore o uguale a 3 (perchè devo considerare anche la bandierina per la cella_max che ancora non ho messo)

    ; con il fatto qui sotto mi prendo tutte le coordinate di tutte le celle_adiacenti_a_cella_max:
    (celle_adiacenti_a_cella_max (x_row_sotto_1 ?x_row_sotto_1) (x_row_sotto_2 ?x_row_sotto_2)
                                 (y_col_sinistra_1 ?y_col_sinistra_1) (y_col_sinistra_2 ?y_col_sinistra_2)
                                 (y_col_destra_1 ?y_col_destra_1) (y_col_destra_2 ?y_col_destra_2))



    ; 3) E inoltre mi devo assicurare che nella riga subito sopra rispetto a cella_max possa mettere 
    ;    la bandierina che supporremo corrisponda al primo "middle" partendo dal basso e quindi deve essere vero che:
    (k-per-row (row ?x_row_sotto_1) (num ?max_pezzi_row_sopra_1))
    (k-per-row-bandierine-posizionate (row ?x_row_sotto_1) (num ?num_b_row_sopra_1))
    (test (>= (- ?max_pezzi_row_sopra_1 ?num_b_row_sopra_1) 1))


    ; 4) E inoltre mi devo assicurare che due righe subito a cella_max possa mettere 
    ;    la bandierina che supporremo corrisponda al secondo “middle” partendo dal basso e quindi deve essere vero che:
    (k-per-row (row ?x_row_sotto_2) (num ?max_pezzi_row_sopra_2))
    (k-per-row-bandierine-posizionate (row ?x_row_sotto_2) (num ?num_b_row_sopra_2))
    ...
    (not (k_cell_agent (x ?x_row_sotto_2) (y ?y) (content ?content) (considerato true) (current no)))

    ; evita che la regola corrente riscatti quando il fatto qui sotto è presente in WM:
    (nave_piazzata_gestore (piazzamento false))
=>

    ; A QUESTO PUNTO SONO CERTO DI POTER PIAZZARE UNA incrociatore nelle seguenti posizioni
    ; (x,y)(dove sappiamo esserci cella_max) - (x-1,y) (primo middle) – (x-2,y)(altro middle) – (x-3,y)(top) 
    ; e quindi faccio questa assert:
    (assert (piazzamento_incrociatore_verticale (x ?x_row_sotto_2) (y ?y))) ; per semplicità qui setto solamente la cella da dove partirà la incrociatore (dal basso)
)



;; DA QUI PARTONO I CONTROLLI SULLE DIREZIONI ORIZZONTALI.

(defrule posizionamento_incrociatore_in_orizzontale_supponendo_cella_max_right (declare (salience 26))
    ...
    (test (>= (- ?max_pezzi_row ?num_b_row) 3)) ; controllo se la differenza è maggiore o uguale a 3 (perchè devo considerare anche la bandierina per la cella_max che ancora non ho messo)
    ...
    ; 3) E inoltre mi devo assicurare che nella colonna subito a sinistra di cella_max possa mettere 
    ...
    ; 4) E inoltre mi devo assicurare che due colonne subito a sinistra possa mettere 
    ;    la bandierina che supporremo corrisponda al secondo “middle” partendo da destra e quindi deve essere vero che:
    (k-per-col (col ?y_col_sinistra_2) (num ?max_pezzi_col_sinistra_2))
    (k-per-col-bandierine-posizionate (col ?y_col_sinistra_2) (num ?num_b_col_sinistra_2))
    (test (>= (- ?max_pezzi_col_sinistra_2 ?num_b_col_sinistra_2) 1))
    ...
    (not (k-cell (x ?x) (y ?y_col_sinistra_2) (content water)))

    ; verifico che in tutte le celle dove posizionerò la nave l'agente non abbia già posizionato
    ; una bandierina:
    (not (k_cell_agent (x ?x) (y ?y) (content ?content) (considerato true) (current no)))
    (not (k_cell_agent (x ?x) (y ?y_col_sinistra_1) (content ?content) (considerato true) (current no)))
    (not (k_cell_agent (x ?x) (y ?y_col_sinistra_2) (content ?content) (considerato true) (current no)))
...
=>
    ...
    ; A QUESTO PUNTO SONO CERTO DI POTER PIAZZARE UNA incrociatore nelle seguenti posizioni
    ; (x,y-3)(left) - (x,y-2) (altro middle) – (x,y-1)(primo middle) – (x,y)(dove sappiamo esserci cella_max) 
    ; e quindi faccio questa assert:
    (assert (piazzamento_incrociatore_orizzontale_cella_max_right (x ?x) (y ?y_col_sinistra_2))) ; per semplicità qui setto solamente la cella da dove partirà la incrociatore (da sinistra)
)



(defrule posizionamento_incrociatore_in_orizzontale_supponendo_cella_max_left (declare (salience 26))
    ...
    (not (k_cell_agent (x ?x) (y ?y_col_destra_2) (content ?content) (considerato true) (current no)))
...
=>
    ...
    ; A QUESTO PUNTO SONO CERTO DI POTER PIAZZARE UNA incrociatore nelle seguenti posizioni
    ; (x,y)(dove sappiamo esserci cella_max) - (x,y+1) (primo middle) – (x,y+2)(altro middle) – (x,y+3)(right) 
    ; e quindi faccio questa assert:
    (assert (piazzamento_incrociatore_orizzontale_cella_max_left (x ?x) (y ?y))) ; per semplicità qui setto solamente la cella da dove partirà la incrociatore (da sinistra)
)
...
;; (sono mutamente esclusive tra loro):
;; aggiunto 

;; C'ERA UN ERRORE QUI SOTTO E QUINDI L'INCROCIATORE NON LO POSIZIONAVA CORRETTAMENTE..
;; RICONTROLLA GLI INCROCIATORI IN TUTTI GLI ALTRI CASI altrimenti rischi di avere un punteggio più 
;; basso solo per colpa di questi dettagli...

;; c'era due volte piazzamento_incrociatore_orizzontale_cella_max_right in tutti e due le reg di sotto
;; senza il not e inoltre c'era un altro errore che riguardava le variabili per la x e la y... 
;; RICONTROLLA BENE QUESTE COSE DOMANI..

;; dopodichè prova a rirunnurare tutto da 80 !!!!!!
(defrule aggiunta_fatti_per_gestire_solo_il_caso_orizzontale_cella_max_right (declare (salience 25))

    (piazzamento_incrociatore_orizzontale_cella_max_right (x ?x) (y ?col_left_caso1))
    (not (piazzamento_incrociatore_orizzontale_cella_max_left (x ?x_left) (y ?col_left_caso2)))

=>  
    (assert (piazzamento_incrociatore_orizzontale(x ?x) (y ?col_left_caso1)))
)
(defrule aggiunta_fatti_per_gestire_solo_il_caso_orizzontale_cella_max_left (declare (salience 25))

    (piazzamento_incrociatore_orizzontale_cella_max_left (x ?x_left) (y ?col_left_caso2))
    (not (piazzamento_incrociatore_orizzontale_cella_max_right (x ?x) (y ?col_left_caso1)))

=>  
    (assert (piazzamento_incrociatore_orizzontale(x ?x_left) (y ?col_left_caso2)))
)
...
;; fact1 -> (piazzamento_incrociatore_orizzontale_cella_max_left (cella(x,y-1))
;; fact2 -> (piazzamento_incrociatore_orizzontale_cella_max_right (cella(x,y-2))
...
(defrule aggiunta_fatti_per_gestione_conflitti_solo_orizzontale_1 (declare (salience 25))

    (piazzamento_incrociatore_orizzontale_cella_max_left (x ?x) (y ?col_left_caso1))
    (piazzamento_incrociatore_orizzontale_cella_max_right (x ?x) (y ?col_left_caso2))
...
=>  
    ...
    (assert (celle_incrociatore_caso1_orizzontale (y_col_left ?col_left_caso1) (y_col_middle_1 (+ ?col_left_caso1 1)) (y_col_right (+ ?col_left_caso1 2))))
    (assert (celle_incrociatore_caso2_orizzontale (y_col_left ?col_left_caso2) (y_col_middle_1 (+ ?col_left_caso2 1)) (y_col_right (+ ?col_left_caso2 2))))
)


(defrule gestione_conflitto_solo_orizzontale (declare (salience 24))

    ; 1) devono essere veri questi fatti in WM
    (piazzamento_incrociatore_orizzontale_cella_max_left (x ?x) (y ?col_left_caso1))
    (piazzamento_incrociatore_orizzontale_cella_max_right (x ?x) (y ?col_left_caso2))

    ; 2) Devo verificare che ?col_sinistra_caso1 e ?col_sinistra_caso2 sono diversi
    ...
    ?celle_incrociatore_caso1 <- (celle_incrociatore_caso1_orizzontale (y_col_left ?col_left_caso1) (y_col_middle_1 ?col_middle1_caso1) (y_col_right ?col_right_caso1))
    ...
    ?celle_incrociatore_caso2 <- (celle_incrociatore_caso2_orizzontale (y_col_left ?col_left_caso2) (y_col_middle_1 ?col_middle1_caso2) (y_col_right ?col_right_caso2))
...
=>  
    ...
    (bind ?score_orizzontale_caso1 (+ ?diff_col_left_caso1 ?diff_col_middle1_caso1 ?diff_col_right_caso1)) ; questo sarà lo score orizzontale del caso 1)
    ...
    (bind ?score_orizzontale_caso2 (+ ?diff_col_left_caso2 ?diff_col_middle1_caso2 ?diff_col_right_caso2)) ; questo sarà lo score orizzontale del caso 2
...
(defrule vince_orizzontale_caso1 (declare (salience 23))

    ; 1) devono essere veri questi fatti in WM
    ?piazzamento_caso1 <- (piazzamento_incrociatore_orizzontale_cella_max_left (x ?x) (y ?col_left_caso1))
    ?piazzamento_caso2 <- (piazzamento_incrociatore_orizzontale_cella_max_right (x ?x) (y ?col_left_caso2))
...
=>  
    ...
    (assert (piazzamento_incrociatore_orizzontale(x ?x) (y ?col_left_caso1))) ; faccio capire quale direzione orizzontale ha vinto ed è quindi quella che rimarrà
    ;(retract ?piazzamento_caso1) ; lo userò per ricordarmi più giù dove si trova il middle
    (retract ?piazzamento_caso2) ; non serve più

)  
(defrule vince_orizzontale_caso2 (declare (salience 23))
...
=>
    (assert (piazzamento_incrociatore_orizzontale(x ?x) (y ?col_left_caso2))) ; faccio capire quale direzione orizzontale ha vinto ed è quindi quella che rimarrà
...
;   -	(piazzamento_incrociatore_orizzontale (cella(x,y))”  
...
=>  
    ...
    (assert (celle_incrociatore_orizzontale (y_col_left ?col_left) (y_col_middle_1 (+ ?col_left 1)) (y_col_right (+ ?col_left 2))))
    (assert (celle_incrociatore_verticale (x_row_bot ?row_bot) (x_row_middle_1 (- ?row_bot 1)) (x_row_top (- ?row_bot 2))))
)
...
(defrule vince_verticale (declare (salience 20))

    ?f_scores <-(scores (orizzontale ?score_orizzontale) (verticale ?score_verticale))

    (test (< ?score_orizzontale ?score_verticale))

    ?f_piazzamento_orizzontale <- (piazzamento_incrociatore_orizzontale (x ?x) (y ?col_su))
...
(defrule vince_orizzontale_supponendo_cella_max_left (declare (salience 20))

    ?f_scores <- (scores (orizzontale ?score_orizzontale) (verticale ?score_verticale))
    (test (>= ?score_orizzontale ?score_verticale))
    
    ?f_piazzamento_orizzontale <- (piazzamento_incrociatore_orizzontale (x ?x) (y ?col_left))
    ?f_piazzamento_verticale <- (piazzamento_incrociatore_verticale (x ?row_bot) (y ?y))
    ...
    (piazzamento_incrociatore_orizzontale_cella_max_left (x ?x) (y ?col_left))
...
=>
    ...
    (assert (k_cell_agent (x ?x) (y (+ ?col_left 2)) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che devo posizionare una bandierina in cella(x,y+2)
    
    (modify ?nave_piazzata (piazzamento true)) ; per far capire al chiamante che una nave è stata piazzata
)


(defrule vince_orizzontale_supponendo_cella_max_right (declare (salience 20))
    ...
    (piazzamento_incrociatore_orizzontale_cella_max_right (x ?x) (y ?col_left))
...
=>
	...
	; posso aggiornare la nostra struttura dove manteniamo aggiornate le posizioni nelle quali
	; l'agente posiziona le sue bandierine:
	(assert (k_cell_agent (x ?x) (y ?col_left) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che devo posizionare una bandierina in cella(x,y-3)
    (assert (k_cell_agent (x ?x) (y (+ ?col_left 1)) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che devo posizionare una bandierina in cella(x,y-2)
    (assert (k_cell_agent (x ?x) (y (+ ?col_left 2)) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che devo posizionare una bandierina in cella(x,y-1)
...
(defrule vince_verticale_senza_conflitto (declare (salience 10)) ; è mutuamente esclusiva con quella di sotto

    (not (piazzamento_incrociatore_orizzontale (x ?x) (y ?y_col_left))) ; aggiunto
    ?f_piazzamento_verticale <- (piazzamento_incrociatore_verticale (x ?x_row_bot) (y ?y))
    
	(status (step ?s)(currently running))
	(not (exec  (action guess) (x ?x_row_bot) (y ?y))) ; verifico di non aver già posizionato una bandierina in questa posizione
...
=>
	...
	; posso aggiornare la nostra struttura dove manteniamo aggiornate le posizioni nelle quali
	; l'agente posiziona le sue bandierine:
	(assert (k_cell_agent (x ?x_row_bot) (y ?y) (content sconosciuto) (considerato false) (current yes))) ; questa è la cella_max nella quale sto assumendo che ci sia il bot in cella(x,y)
    (assert (k_cell_agent (x (- ?x_row_bot 1)) (y ?y) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che devo posizionare una bandierina in cella(x-1,y)
    (assert (k_cell_agent (x (- ?x_row_bot 2)) (y ?y) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che devo posizionare una bandierina in cella(x-2,y)
    
    (modify ?nave_piazzata (piazzamento true)) ; per far capire al chiamante che una nave è stata piazzata

)
(defrule vince_orizzontale_supponendo_cella_max_left_senza_conflitto (declare (salience 10))

    (not (piazzamento_incrociatore_verticale (x ?row_bot) (y ?y))) ; aggiunto
    ?f_piazzamento_orizzontale <- (piazzamento_incrociatore_orizzontale_cella_max_left (x ?x) (y ?col_left))
...
(defrule vince_orizzontale_supponendo_cella_max_right_senza_conflitto (declare (salience 10))

    (not (piazzamento_incrociatore_verticale (x ?row_bot) (y ?y))) ; aggiunto
    ?f_piazzamento_orizzontale <- (piazzamento_incrociatore_orizzontale_cella_max_right (x ?x) (y ?col_left))
...
; Con la regola di sotto l'agente si memorizza nella sua struttura "incrociatore" sia le celle nelle quali 
; ha deciso di posizionare la incrociatore e sia il fatto che adesso gli manca ancora da cercare "?m - 1" incrociatore. 
(defrule memorizzo_incrociatore_1 (declare (salience 3))
	...
	?incrociatore <- (incrociatori (celle_con_bandierina $?lista) (mancanti ?m))
    ...
    ?decremento_incrociatore <- (decremento_incrociatori (cella_incrociatore_1 false) (cella_incrociatore_2 false) (cella_incrociatore_3 false))
=>
	(modify ?incrociatore (celle_con_bandierina (insert$ $?lista (+ (length$ $?lista) 1) ?new_cella)))
    ...
    (modify ?decremento_incrociatore (cella_incrociatore_1 true)) ; attivo la regola "decremento_incrociatore" qui sotto
...
(defrule memorizzo_incrociatore_2 (declare (salience 3))
	...
	?k_row_bandierine <- (k-per-row-bandierine-posizionate (row ?x) (num ?num_b_row))
	?k_col_bandierine <- (k-per-col-bandierine-posizionate (col ?y) (num ?num_b_col))
    ?decremento_incrociatore <- (decremento_incrociatori (cella_incrociatore_1 true) (cella_incrociatore_2 false) (cella_incrociatore_3 false))
=>
	(modify ?incrociatore (celle_con_bandierina (insert$ $?lista (+ (length$ $?lista) 1) ?new_cella)))
	(modify ?k_row_bandierine (num (+ ?num_b_row 1))) ; aggiungo una bandierina in riga ?x
	(modify ?k_col_bandierine (num (+ ?num_b_col 1))) ; aggiungo una bandierina in colonna ?y
    (modify ?decremento_incrociatore (cella_incrociatore_2 true)) ; attivo la regola "decremento_incrociatore" qui sotto
    (modify ?new_cella (considerato true)) 
)
(defrule memorizzo_incrociatore_3 (declare (salience 3))
	...
	?k_row_bandierine <- (k-per-row-bandierine-posizionate (row ?x) (num ?num_b_row))
	?k_col_bandierine <- (k-per-col-bandierine-posizionate (col ?y) (num ?num_b_col))
    ?decremento_incrociatore <- (decremento_incrociatori (cella_incrociatore_1 true) (cella_incrociatore_2 true) (cella_incrociatore_3 false))
=>
	(modify ?incrociatore (celle_con_bandierina (insert$ $?lista (+ (length$ $?lista) 1) ?new_cella)))
	(modify ?k_row_bandierine (num (+ ?num_b_row 1))) ; aggiungo una bandierina in riga ?x
	(modify ?k_col_bandierine (num (+ ?num_b_col 1))) ; aggiungo una bandierina in colonna ?y
    (modify ?decremento_incrociatore (cella_incrociatore_3 true)) ; attivo la regola "decremento_incrociatore" qui sotto
    (modify ?new_cella (considerato true)) 
)


(defrule decremento_incrociatore (declare (salience 2))
	?decremento_incrociatore <- (decremento_incrociatori (cella_incrociatore_1 true) (cella_incrociatore_2 true) (cella_incrociatore_3 true)) ; tutti e 4 gli slot devono essere a true
    ?incrociatore <- (incrociatori (celle_con_bandierina $?lista) (mancanti ?m))
=>
	(modify ?incrociatore (mancanti (- ?m 1)))
    (retract ?decremento_incrociatore)
)
...
(defrule cancello_celle_adiacenti_a_cella_max_ormai_inutili(declare (salience 1))
    ...
    ?cella_da_cancellare <- (celle_adiacenti_a_cella_max (x_row_sotto_1 ?x_row_sotto_1) (x_row_sotto_2 ?x_row_sotto_2)
                                                         (y_col_sinistra_1 ?y_col_sinistra_1) (y_col_sinistra_2 ?y_col_sinistra_2)
                                                         (y_col_destra_1 ?y_col_destra_1) (y_col_destra_2 ?y_col_destra_2))
