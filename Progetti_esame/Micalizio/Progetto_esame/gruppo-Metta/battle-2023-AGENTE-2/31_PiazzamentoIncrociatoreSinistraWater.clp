...
(defmodule PIAZZAMENTO_INCROCIATORE_SINISTRA_WATER (import MAIN ?ALL) (import ENV ?ALL) (import AGENT ?ALL) (import GESTORE_SINISTRA_WATER ?ALL) (export ?ALL))
...
;; 1-in orizzontale (ovviamente VERSO sinistra)
...
(deftemplate celle_adiacenti_a_cella_max  
    ...
    (slot x_row_sotto_2) ; riga +2 sotto cella_max
    
    (slot y_col_sinistra_1) ; colonna subito a sinistra della cella_max
    (slot y_col_sinistra_2) ; colonna +2 a sinistra della cella_max
)
...
(deftemplate celle_incrociatore_caso1_verticale
    (slot x_row_bot) 
    (slot x_row_middle_1) 
    (slot x_row_top) 
)
(deftemplate celle_incrociatore_caso2_verticale
    (slot x_row_bot) 
    (slot x_row_middle_1) 
    (slot x_row_top) 
)
...
(deftemplate piazzamento_incrociatore_verticale_cella_max_bot
    (slot x)
    (slot y)
)
(deftemplate piazzamento_incrociatore_verticale_cella_max_top
    (slot x)
    (slot y)
)
(deftemplate piazzamento_incrociatore_verticale ;; in caso di conflitti sulla dir verticale, conterrà comunque il punto di partenda della incrociatore che ha vinto lungo la dir verticale
    (slot x)
    (slot y)
)
...
(defrule aggiunta_fatti_per_reg_successive (declare (salience 50))
    (cella_max (x ?x) (y ?y) (direzione sinistra))
=>
    (assert (celle_adiacenti_a_cella_max (x_row_sopra_1 (- ?x 1)) (x_row_sopra_2 (- ?x 2)) 
                                         (x_row_sotto_1 (+ ?x 1)) (x_row_sotto_2 (+ ?x 2))  
...
(defrule posizionamento_incrociatore_in_orizzontale_supponendo_cella_max_right (declare (salience 26))
    ...
    (celle_adiacenti_a_cella_max (x_row_sopra_1 ?x_row_sopra_1) (x_row_sopra_2 ?x_row_sopra_2)
                                 (x_row_sotto_1 ?x_row_sotto_1) (x_row_sotto_2 ?x_row_sotto_2)
...
=>
    ...
    (assert (piazzamento_incrociatore_orizzontale (x ?x) (y ?y_col_sinistra_2))) ; per semplicità qui setto solamente la cella da dove partirà l'incrociatore (da sinistra)
)
...
=>
    ...
    ; e quindi faccio questa assert:
    (assert (piazzamento_incrociatore_verticale_cella_max_bot (x ?x) (y ?y))) ; per semplicità qui setto solamente la cella da dove partirà la incrociatore (dal basso)
)
...
(defrule posizionamento_incrociatore_in_verticale_supponendo_cella_max_top (declare (salience 26))
    ...
    ; 3) E inoltre mi devo assicurare che nella riga subito sotto al bot possa mettere 
    ;    la bandierina che supporremo corrisponda al "middle" partendo dal basso e quindi deve essere vero che:
...
=>
    ...
    ; A QUESTO PUNTO SONO CERTO DI POTER PIAZZARE UNA incrociatore nelle seguenti posizioni
    ; (x-2,y)(bot) - (x-1,y) (middle) – (x,y)(dove sappiamo esserci cella_max)
    ; e quindi faccio questa assert:
    (assert (piazzamento_incrociatore_verticale_cella_max_top (x ?x_row_sotto_2) (y ?y))) ; per semplicità qui setto solamente la cella da dove partirà la incrociatore (dal basso)
)
...
;; aggiunto 
(defrule aggiunta_fatti_per_gestire_solo_il_caso_verticale_cella_max_bot (declare (salience 25))

    (piazzamento_incrociatore_verticale_cella_max_bot (x ?x_row_bot_caso1) (y ?y_caso1))
    (not (piazzamento_incrociatore_verticale_cella_max_top (x ?x_row_bot_caso2) (y ?y_caso2)))

=>  
    (assert (piazzamento_incrociatore_verticale(x ?x_row_bot_caso1) (y ?y_caso1)))
)
(defrule aggiunta_fatti_per_gestire_solo_il_caso_verticale_cella_max_top (declare (salience 25))

    (piazzamento_incrociatore_verticale_cella_max_top (x ?x_row_bot_caso2) (y ?y_caso2))
    (not (piazzamento_incrociatore_verticale_cella_max_bot (x ?x_row_bot_caso1) (y ?y_caso1)))

=>  
    (assert (piazzamento_incrociatore_verticale(x ?x_row_bot_caso2) (y ?y_caso2)))
)
...
;; fact1 -> (piazzamento_incrociatore_verticale (cella(x,y-1))
;; fact2 -> (piazzamento_incrociatore_verticale (cella(x,y-2))
...
(defrule aggiunta_fatti_per_gestione_conflitti_solo_verticale (declare (salience 25))
    ...
    (piazzamento_incrociatore_verticale_cella_max_bot (x ?row_bot_caso1) (y ?y))
    (piazzamento_incrociatore_verticale_cella_max_top (x ?row_bot_caso2) (y ?y))
...
=>  
    ...
    (assert (celle_incrociatore_caso1_verticale (x_row_bot ?row_bot_caso1) (x_row_middle_1 (- ?row_bot_caso1 1)) (x_row_top (- ?row_bot_caso1 2))))
    (assert (celle_incrociatore_caso2_verticale (x_row_bot ?row_bot_caso2) (x_row_middle_1 (- ?row_bot_caso2 1)) (x_row_top (- ?row_bot_caso2 2))))
)
...
(defrule gestione_conflitto_solo_verticale (declare (salience 24))
    ...
    ?celle_incrociatore_caso1 <- (celle_incrociatore_caso1_verticale (x_row_bot ?row_bot_caso1) (x_row_middle_1 ?row_middle1_caso1) (x_row_top ?row_right_caso1))
    ...
    ?celle_incrociatore_caso2 <- (celle_incrociatore_caso2_verticale (x_row_bot ?row_bot_caso2) (x_row_middle_1 ?row_middle1_caso2) (x_row_top ?row_right_caso2))
...
=>  
    ...
    (bind ?score_verticale_caso1 (+ ?diff_row_bot_caso1 ?diff_row_middle1_caso1 ?diff_row_right_caso1)) ; questo sarà lo score verticale del caso 1
    ...
    (bind ?score_verticale_caso2 (+ ?diff_row_bot_caso2 ?diff_row_middle1_caso2 ?diff_row_right_caso2)) ; questo sarà lo score verticale del caso 2
...
(defrule vince_verticale_caso1 (declare (salience 23))
    ...
    ; 1) devono essere veri questi fatti in WM
    ?piazzamento_caso1 <- (piazzamento_incrociatore_verticale_cella_max_bot (x ?row_bot_caso1) (y ?y))
    ?piazzamento_caso2 <- (piazzamento_incrociatore_verticale_cella_max_top (x ?row_bot_caso2) (y ?y))
...
=>
    (assert (piazzamento_incrociatore_verticale(x ?row_bot_caso1) (y ?y))) ; faccio capire quale direzione verticale ha vinto ed è quindi quella che rimarrà
...
=>
    (assert (piazzamento_incrociatore_verticale(x ?row_bot_caso2) (y ?y))) ; faccio capire quale direzione verticale ha vinto ed è quindi quella che rimarrà
...
=>
    ...
    (assert (k_cell_agent (x ?x) (y (+ ?col_left 2)) (content sconosciuto) (considerato false) (current yes))) ; questa è la cella_max nella quale sto assumendo che ci sia il right in cella(x,y)
...
(defrule vince_verticale_supponendo_cella_max_bot (declare (salience 20))
    ...
    (piazzamento_incrociatore_verticale_cella_max_bot (x ?x) (y ?y)) ; punto di mutua esclusione
...
(defrule vince_verticale_supponendo_cella_max_top (declare (salience 20))
    ...
    (piazzamento_incrociatore_verticale_cella_max_top (x ?x) (y ?y)) ; punto di mutua esclusione
...
(defrule vince_orizzontale_senza_conflitto (declare (salience 10)) ; è mutuamente esclusiva con quella di sotto
    ...
    (not (piazzamento_incrociatore_verticale (x ?x_row_bot) (y ?y))) ; aggiunto
    ?f_piazzamento_orizzontale <- (piazzamento_incrociatore_orizzontale (x ?x) (y ?col_left))
...
=>
	...
	; posso aggiornare la nostra struttura dove manteniamo aggiornate le posizioni nelle quali
	; l'agente posiziona le sue bandierine:
	(assert (k_cell_agent (x ?x) (y ?col_left) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che devo posizionare una bandierina in cella(x,y-21)
    (assert (k_cell_agent (x ?x) (y (+ ?col_left 1)) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che devo posizionare una bandierina in cella(x,y-1)
    (assert (k_cell_agent (x ?x) (y (+ ?col_left 2)) (content sconosciuto) (considerato false) (current yes))) ; questa è la cella_max nella quale sto assumendo che ci sia il right in cella(x,y)
...
(defrule vince_verticale_supponendo_cella_max_bot_senza_conflitto (declare (salience 10))
    ...
    ?f_piazzamento_verticale <- (piazzamento_incrociatore_verticale_cella_max_bot (x ?row_bot) (y ?y))
...
(defrule vince_verticale_supponendo_cella_max_top_senza_conflitto (declare (salience 10))
    ...
    (not (piazzamento_incrociatore_orizzontale (x ?x) (y ?y_col_left))) ; aggiunto
    ?f_piazzamento_verticale <- (piazzamento_incrociatore_verticale_cella_max_top (x ?row_bot) (y ?y))
