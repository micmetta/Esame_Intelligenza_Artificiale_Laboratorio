...
(defmodule PIAZZAMENTO_INCROCIATORE_RIGHT (import MAIN ?ALL) (import ENV ?ALL) (import AGENT ?ALL) (import GESTORE_RIGHT ?ALL) (export ?ALL))

; Questo modulo prova a piazzare una incrociatore solo in verticale partendo dalla cella "right" conosciuta e 
...
(defrule posizionamento_incrociatore_in_orizzontale_conoscendo_right (declare (salience 26))
...
=>
    ...
    ; (x,y-2)(left) - (x,y-1)(middle) – (x,y)(dove sappiamo esserci il right) 
    ; e quindi faccio questa assert:
    (assert (piazzamento_incrociatore_orizzontale (x ?x) (y ?y_col_sinistra_2))) ; per semplicità qui setto solamente la cella da dove partirà l'incrociatore (da sinistra ovvero da dove supponiamo esserci il left)
)
...
=>
	...
	(assert (k_cell_agent (x ?x) (y ?y_col_left) (content sconosciuto) (considerato false) (current yes))) ; ; mi ricordo che devo posizionare una bandierina in cella(x,y-2)
    ...
    ;(assert (k_cell_agent (x ?x) (y (+ ?y_col_left 2)) (content right) (considerato false) (current yes))) ; QUI E' DOVE C'E' IL RIGHT (cella(x,y) E QUINDI QUESTA ASSERT NON SERVE (perchè già è presente in WM la k_cell_agent corrispondente)
...
(defrule memorizzo_incrociatore_2 (declare (salience 3))
    ?new_cella <- (k_cell_agent (x ?x) (y ?y) (content right) (considerato false) (current yes)) ; il "(content left)" lo metto in qui e non sopra perchè in questo modo sono certo di entrare nella reg di memorizzazione_2 solamente dopo che sono entrato nella prima (cioè quella di sopra)
	?incrociatori <- (incrociatori (celle_con_bandierina $?lista) (mancanti ?m))
    ?k_row_bandierine <- (k-per-row-bandierine-posizionate (row ?x) (num ?num_b_row))
	?k_col_bandierine <- (k-per-col-bandierine-posizionate (col ?y) (num ?num_b_col))
