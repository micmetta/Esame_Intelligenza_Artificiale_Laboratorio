...
(deftemplate decremento_corazzate
    ...
    ; la cella che contiene il primo pezzo della corazzaza (true) o meno (false)
...
; template che permette di poter specificare se bisogna esegure o meno 
; l'azione di unguess
...
(defrule non_sono_riuscito_a_piazzare_nessuna_nave_precedente (declare (salience 100))
...
=>
    ...
    ;; chiamata "eseguo_unguess_lungo_....":
...
;; la colonna "y" varia (perchè stiamo trattando un top):
(defrule eseguo_unguess_su_due_righe_sotto_a_cella_top (declare (salience 99))
...
;; la colonna "y" varia (perchè stiamo trattando un top):
(defrule eseguo_unguess_su_tre_righe_sotto_a_cella_top (declare (salience 99))
...
;; Inoltre, il contatore ?m verrà incrementato di 1 perchè la corazzata precedente era sbagliata e quindi bisognerà ancora
...
(defrule controllo_incremento_sottomarini_da_trovare (declare (salience 98))
    ...
    (not (sottomarini_incrementati (x ?x) (y ?y) (incrementata true)))
