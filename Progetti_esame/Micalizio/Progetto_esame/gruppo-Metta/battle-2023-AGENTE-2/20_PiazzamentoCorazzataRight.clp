...
(defmodule PIAZZAMENTO_CORAZZATA_RIGHT (import MAIN ?ALL) (import ENV ?ALL) (import AGENT ?ALL) (import GESTORE_RIGHT ?ALL) (export ?ALL))

; Questo modulo prova a piazzare una corazzata solo in verticale partendo dalla cella "right" conosciuta e 
...
(defrule posizionamento_corazzata_in_orizzontale_conoscendo_right (declare (salience 26))
    ...
    (not(k_cell_agent (x ?x) (y ?y_col_sinistra_2) (content sconosciuto) (considerato true) (current no)))
    (not(k_cell_agent (x ?x) (y ?y_col_sinistra_3) (content sconosciuto) (considerato true) (current no)))
...
=>
    ...
    ; (x,y-3)(left) - (x,y-3) (secondo middle) – (x,y-2)(primo middle) – (x,y)(dove sappiamo esserci il right) 
    ; e quindi faccio questa assert:
    (assert (piazzamento_corazzata_orizzontale (x ?x) (y ?y_col_sinistra_3))) ; per semplicità qui setto solamente la cella da dove partirà la corazzata (da sinistra ovvero da dove supponiamo esserci il left)
)
...
=>
	...
	(assert (k_cell_agent (x ?x) (y ?y_col_left) (content sconosciuto) (considerato false) (current yes))) ; ; mi ricordo che devo posizionare una bandierina in cella(x,y-3)
    (assert (k_cell_agent (x ?x) (y (+ ?y_col_left 1)) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che devo posizionare una bandierina in cella(x,y-2)
    (assert (k_cell_agent (x ?x) (y (+ ?y_col_left 2)) (content sconosciuto) (considerato false) (current yes))) ; mi ricordo che ho posizionato una bandierina in cella(x,y-1)
    ;(assert (k_cell_agent (x ?x) (y (+ ?y_col_left 3)) (content right) (considerato false) (current yes))) ; QUI E' DOVE C'E' IL RIGHT (cella(x,y) E QUINDI QUESTA ASSERT NON SERVE (perchè già è presente in WM la k_cell_agent corrispondente)
