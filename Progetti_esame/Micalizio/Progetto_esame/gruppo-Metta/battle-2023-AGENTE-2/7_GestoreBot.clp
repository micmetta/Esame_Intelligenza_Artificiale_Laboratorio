...
(defmodule GESTORE_BOT (import MAIN ?ALL) (import ENV ?ALL) (import AGENT ?ALL) (export ?ALL))

; Questo modulo si preoccuperà di chiamare prima il modulo "PIAZZAMENTO_CORAZZATA" e se questo non riesce in nessun
; modo a piazzare una corazzata (conoscendo che in cella(x,y) c'è un bot) allora richiamerà il modulo
...
(deftemplate celle_sopra_a_bot_in_gestore_bot
...
; sono riusciti a piazzare una nave perchè con la regola di sotto il modulo GESTORE_BOT 
...
(defrule controllo_piazzamento_nave_gestore_bot (declare (salience 170))
...
;; regola qui sotto per ritrattare la cella_bot ormai risolta e anche il fatto "celle_sopra_a_bot_in_gestore_bot" 
;; perchè ormai non serve più:
(defrule cancellazione_cella_bot_risolta (declare (salience 169))
    
    (nave_piazzata_gestore (piazzamento true)) ; controllo se sono riuscito a piazzare una nave
    
    ; Aggancio la cella_bot_corrente che il modulo AGENT mi ha detto di considerare
    ; e per la quale l'agente è riscito a piazzare una nave:
    ?cella_bot_corrente <- (cella_bot (x ?x) (y ?y) (considerata true))

    ; aggiunta..
    ?celle_sopra_a_bot_in_gestore_bot <- (celle_sopra_a_bot_in_gestore_bot (x_row_sopra_1 ?x_row_sopra_1) (x_row_sopra_2 ?x_row_sopra_2) (x_row_sopra_3 ?x_row_sopra_3))
=>
    (retract ?cella_bot_corrente) ;; ormai l'ho completata quindi la tolgo in modo tale che l'agente non la rivaluti
    
    ; aggiunta..
    (retract ?celle_sopra_a_bot_in_gestore_bot) ;; ormai non mi serve più
)


;; LA REGOLA QUI SOTTO VERRA' ESEGUITA SOLAMENTE SE L'AGENTE NON E' RIUSCITO A PIAZZARE NULLA E 
;; si trova nella fase 3:
;;(defrule controllo_nessun_piazzamento_nave_gestore_bot (declare (salience 168))
...
;;    (nessuna_nave_posizionata (posizionamento nessuno))
    ...
    ; Aggancio la cella_bot_corrente che il modulo AGENT mi ha detto di considerare
    ; e per la quale l'agente è riscito a piazzare una nave
;;    ?cella_bot_corrente <- (cella_bot (x ?x) (y ?y) (considerata false))
;;    ?new_cella <- (k_cell_agent (x ?x) (y ?y) (content bot) (considerato false) (current yes))

;;=>
;;    (retract ?cella_bot_corrente) ;; ormai ci ho provato quindi la tolgo in modo tale che l'agente non la rivaluti
...
(defrule controllo_piazzamento_cacciatorpediniere (declare (salience 150))
    ...
    (cella_bot (x ?x) (y ?y) (considerata true)) ; controllo che ci sia una cella_bot che il modulo AGENT mi ha detto di considerare
...
=>  
    ...
    (focus PIAZZAMENTO_CACCIATORPEDINIERE_BOT)
)
...
=>  
    ...
    ; richiamo il sotto-modulo che si preoccuperà di posizionare l'incrociatore
    (focus PIAZZAMENTO_INCROCIATORE_BOT)
)
...
=>  
    ...
    (focus PIAZZAMENTO_CORAZZATA_BOT)
)
...
(defrule controllo_se_non_sono_riuscito_a_piazzare_nessuna_nave_precedente (declare (salience 100))
    ...
    ; controllo che ci sia una cella_bot che il modulo AGENT mi ha detto di considerare:
    ?cella_bot_corrente <- (cella_bot (x ?x) (y ?y) (considerata true))
...
=>  
    ...
    ;; partendo da una cella nella quale è certo che ci sia il "bot" di una nave.
...
;; la colonna "y" rimane fissa e controllo se lungo questa colonna devo fare l'unguess (perchè stiamo trattando un bot):
(defrule eseguo_unguess_lungo_la_stessa_colonna_di_cella_bot (declare (salience 99))

    ?cella_bot_corrente <- (cella_bot (x ?x) (y ?y) (considerata true)) ; prendo le info della cella che mi ha 
    ...
    ; verifico di NON riuscire a piazzare una bandierina nella colonna in cui si trova la cella bot:
    ...
    ; (aggiunta)
    (exec (step ?s) (action guess) (x ?x_var) (y ?y))
...
=>
    ...
    ; asserisco la cella da cancellare (in questo caso sarà quella presente nella riga subito sopra alla cella bot
...
;; la colonna "y" varia e considero come riga "x" quella subito sopra a dove si trova cella_bot (perchè stiamo trattando un bot):
(defrule eseguo_unguess_su_riga_sopra_a_cella_bot (declare (salience 99))
    ...
    ; verifico di riuscire a piazzare una bandierina nella colonna in cui si trova la cella bot:
    ...
    ; verifico di non riuscire a piazzare una bandierina nella riga subito sopra alla cella bot:
    ...
    ; mi assicuro che sulla cella sulla quale farò l'unguess l'agente abbia già posizionato una bandierina:
    ; (aggiunta)
    (exec (step ?s) (action guess) (x ?x_row_sopra_1) (y ?y_var))

    ; A questo punto sono certo che devo togliere una bandierina dalla "x_row_sopra_1" dove però
    ; la colonna non dovrà essere uguale ad "y" e quindi
    ; prendo la posizione di una delle bandierine che l'agente ha posizionato lungo
    ; la riga "x_row_sopra_1" (la "y" invece può essere anche diversa da quella della cella bot)
    ?k_cell_agent <- (k_cell_agent (x ?x_row_sopra_1) (y ?y_var) (content ?val_content) (considerato ?val_considerato) (current ?val_current))
    ...
    ; mi assicuro che la bandierina che andrò a togliere non faccia riferimento ad un fatto iniziale che 
    ; mi assicura che in quella posizione ci sia un qualche pezzo di una nave:
    (not (k-cell (x ?x_row_sopra_1) (y ?y_var) (content top)))
    (not (k-cell (x ?x_row_sopra_1) (y ?y_var) (content bot)))
    (not (k-cell (x ?x_row_sopra_1) (y ?y_var) (content left)))
    (not (k-cell (x ?x_row_sopra_1) (y ?y_var) (content right)))
    (not (k-cell (x ?x_row_sopra_1) (y ?y_var) (content middle)))
    (not (k-cell (x ?x_row_sopra_1) (y ?y_var) (content sub)))

    ?posizione_bandierina_da_togliere <- (k-per-row-bandierine-posizionate (row ?x_row_sopra_1) (num ?n1)) 
...
=>
    ...
    ; asserisco la cella da cancellare (in questo caso sarà quella presente nella riga subito sopra alla cella bot
    ; per questo motivo c'è "?row_sopra_1"):
    (assert (bandierina_da_cancellare (x ?x_row_sopra_1) (y ?y_var) (content sconosciuto) (considerato true) (current yes)))
...
;; la colonna "y" varia (perchè stiamo trattando un bot):
(defrule eseguo_unguess_su_due_righe_sopra_a_cella_bot (declare (salience 99))
    ...
    ; verifico di non riuscire a piazzare una bandierina nella riga due_volte_sopra (quindi -2) alla cella bot:
    ; (questo controllo mi permette di sapere con certezza che la bandierina da togliere deve essere una
    ;  di quelle presente proprio in questa riga!)
    (k-per-row (row ?x_row_sopra_2) (num ?max_pezzi_row_sopra_1))
    (k-per-row-bandierine-posizionate (row ?x_row_sopra_2) (num ?num_b_row_sopra_1))
    (test (< (- ?max_pezzi_row_sopra_1 ?num_b_row_sopra_1) 1))

    ; mi assicuro che sulla cella sulla quale farò l'unguess l'agente abbia già posizionato una bandierina:
    ; (aggiunta)
    (exec (step ?s) (action guess) (x ?x_row_sopra_2) (y ?y_var))

    ; A questo punto sono certo che devo togliere una bandierina dalla "x_row_sopra_2" dove però
    ; la colonna non dovrà essere uguale ad "y" e quindi
    ; prendo la posizione di una delle bandierine che l'agente ha posizionato lungo
    ; la riga "x_row_sopra_2" (la "y" invece può essere anche diversa da quella della cella bot)
    ?k_cell_agent <- (k_cell_agent (x ?x_row_sopra_2) (y ?y_var) (content ?val_content) (considerato ?val_considerato) (current ?val_current))
    ...
    ; mi assicuro che la bandierina che andrò a togliere non faccia riferimento ad un fatto iniziale che 
    ; mi assicura che in quella posizione ci sia un qualche pezzo di una nave:
    (not (k-cell (x ?x_row_sopra_2) (y ?y_var) (content top)))
    (not (k-cell (x ?x_row_sopra_2) (y ?y_var) (content bot)))
    (not (k-cell (x ?x_row_sopra_2) (y ?y_var) (content left)))
    (not (k-cell (x ?x_row_sopra_2) (y ?y_var) (content right)))
    (not (k-cell (x ?x_row_sopra_2) (y ?y_var) (content middle)))
    (not (k-cell (x ?x_row_sopra_2) (y ?y_var) (content sub)))

    ?posizione_bandierina_da_togliere <- (k-per-row-bandierine-posizionate (row ?x_row_sopra_2) (num ?n1)) 
    ?posizione_colonna_bandierina_da_togliere <- (k-per-col-bandierine-posizionate (col ?y_var) (num ?n2)) 
...
=>
    ...
    ; asserisco la cella da cancellare (in questo caso sarà quella presente nella riga subito sopra alla cella bot
    ; per questo motivo c'è "?row_sopra_1"):
    (assert (bandierina_da_cancellare (x ?x_row_sopra_2) (y ?y_var) (content sconosciuto) (considerato true) (current yes)))

    ; aggiorno la nostra struttura dell'agente in modo tale da togliere 1 bandierina nella riga "?x_row_sopra_2":
    (modify ?posizione_bandierina_da_togliere (num (- ?n1 1)))
    (modify ?posizione_colonna_bandierina_da_togliere (num (- ?n2 1)))
...
;; la colonna "y" varia (perchè stiamo trattando un bot):
(defrule eseguo_unguess_su_tre_righe_sopra_a_cella_bot (declare (salience 99))
    ...
    ; (questo controllo mi permette di sapere con certezza che la bandierina da togliere deve essere una
    ;  di quelle presente proprio in questa riga!)
    (k-per-row (row ?x_row_sopra_3) (num ?max_pezzi_row_sopra_1))
    (k-per-row-bandierine-posizionate (row ?x_row_sopra_3) (num ?num_b_row_sopra_1))
    (test (< (- ?max_pezzi_row_sopra_1 ?num_b_row_sopra_1) 1))


    ; mi assicuro che sulla cella sulla quale farò l'unguess l'agente abbia già posizionato una bandierina:
    ; (aggiunta)
    (exec (step ?s) (action guess) (x ?x_row_sopra_3) (y ?y_var))

    ; A questo punto sono certo che devo togliere una bandierina dalla "x_row_sopra_3" dove però
    ; la colonna non dovrà essere uguale ad "y" e quindi
    ; prendo la posizione di una delle bandierine che l'agente ha posizionato lungo
    ; la riga "x_row_sopra_3" (la "y" invece può essere anche diversa da quella della cella bot)
    ?k_cell_agent <- (k_cell_agent (x ?x_row_sopra_3) (y ?y_var) (content ?val_content) (considerato ?val_considerato) (current ?val_current))
    ...
    ; mi assicuro che la bandierina che andrò a togliere non faccia riferimento ad un fatto iniziale che 
    ; mi assicura che in quella posizione ci sia un qualche pezzo di una nave:
    (not (k-cell (x ?x_row_sopra_3) (y ?y_var) (content top)))
    (not (k-cell (x ?x_row_sopra_3) (y ?y_var) (content bot)))
    (not (k-cell (x ?x_row_sopra_3) (y ?y_var) (content left)))
    (not (k-cell (x ?x_row_sopra_3) (y ?y_var) (content right)))
    (not (k-cell (x ?x_row_sopra_3) (y ?y_var) (content middle)))
    (not (k-cell (x ?x_row_sopra_3) (y ?y_var) (content sub)))

    ?posizione_bandierina_da_togliere <- (k-per-row-bandierine-posizionate (row ?x_row_sopra_3) (num ?n1)) 
    ?posizione_colonna_bandierina_da_togliere <- (k-per-col-bandierine-posizionate (col ?y_var) (num ?n2)) 
=>

    ; asserisco la cella da cancellare (in questo caso sarà quella presente nella riga subito sopra alla cella bot
    ; per questo motivo c'è "?row_sopra_1"):
    (assert (bandierina_da_cancellare (x ?x_row_sopra_3) (y ?y_var) (content sconosciuto) (considerato true) (current yes)))

    ; aggiorno la nostra struttura dell'agente in modo tale da togliere 1 bandierina nella riga "?x_row_sopra_3":
    (modify ?posizione_bandierina_da_togliere (num (- ?n1 1)))
    (modify ?posizione_colonna_bandierina_da_togliere (num (- ?n2 1)))
...
(defrule controllo_incremento_corazzate_da_trovare (declare (salience 98))
    ...
    ; per la stessa cella bot di partenza
...
=>
    ...
    (retract ?cella_bot) ;; non serve più
    (retract ?fai_unguess)
)