...
(defmodule GESTORE_DESTRA_WATER (import MAIN ?ALL) (import ENV ?ALL) (import AGENT ?ALL) (import GESTORE_WATER ?ALL) (export ?ALL))

; Questo modulo SA' CHE A SINISTRA DELLA (cella_max (x) (y) (direzione destra)) c'è WATER 
; e quindi anche tutti i suoi sotto-moduli di piazzamento sapranno questa cosa e quindi questi ultimi
; cercheranno di piazzare una qualche nave partendo proprio (cella_max (x) (y) (direzione destra))
; SFRUTTANDO L'INFORMAZIONE CHE NELLA CELLA SUBITO A SINISTRA della cella_max C'E' WATER.
...
; sono riusciti a piazzare una nave perchè con la regola di sotto il modulo GESTORE_WATER_DESTRA
...
(defrule controllo_piazzamento_nave_gestore_water_destra (declare (salience 170))
...
(defrule controllo_nessun_piazzamento_nave_gestore_water_destra (declare (salience 168))
...
(defrule controllo_piazzamento_sottomarino (declare (salience 150))
    ...
    (fase_2_iniziata (start true)) ; L'AGENTE SI ASSICURA DI TROVARSI NELLA FASE 2

    (cella_max (x ?x) (y ?y) (direzione destra)) ; controllo che ci sia una cella_middle che il modulo GESTORE_WATER mi ha detto di considerare
 ...
 =>  
    ...
    (focus PIAZZAMENTO_SOTTOMARINO_DESTRA_WATER)
)
...
=>  
    ...
    ; richiamo il sotto-modulo che si preoccuperà di posizionare l'incrociatore
    (focus PIAZZAMENTO_CACCIATORPEDINIERE_DESTRA_WATER)
)
...
=>  
    ...
    ; richiamo il sotto-modulo che si preoccuperà di posizionare l'incrociatore
    (focus PIAZZAMENTO_INCROCIATORE_DESTRA_WATER)
)
...
=>  
    ...
    (focus PIAZZAMENTO_CORAZZATA_DESTRA_WATER)
)
...
(defrule controllo_se_non_sono_riuscito_a_piazzare_nessuna_nave_precedente (declare (salience 100))
    ...
    ?cella_max_corrente <- (cella_max (x ?x) (y ?y) (direzione destra))
...
=>
    ...
    ;; nave è stata posizionata in modo tale che la regola "controllo_nessun_piazzamento_nave_gestore_water_destra"
