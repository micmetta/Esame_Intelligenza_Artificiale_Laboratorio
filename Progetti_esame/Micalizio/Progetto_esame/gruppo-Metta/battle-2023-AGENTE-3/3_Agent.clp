...
(deftemplate corazzata
	;(multislot posizioni_4_celle) ; conterrà le 4 celle di tipo k_cell_bandierina (xi,yi)
	
	(multislot celle_con_bandierina)
	(slot mancanti) ;; mi dirà quante corazzate mi mancano da piazzare (inizio == 4 perchè 4 celle * 1 corazzata)
)
...
;; servirà al modulo GESTORE_FASE_2 per far capire ad Agent in quale cella dovrà eseguire la fire:
...
; La regola di sotto si preoccupa di prendere un fatto iniziale "top" e di metterlo in coda a "conoscenza_ordinata":
...
=>	
	...
	(assert (k_cell_agent (x ?x) (y ?y) (content top) (considerato false) (current yes))) ; voglio inserire in coda a $?x il ?fatto_top
...
;;;;; CON LA REGOLA DI SOTTO VIENE FATTA PARTIRE LA FASE 3 ;;;;;
