...
(defmodule PIAZZAMENTO_INCROCIATORE_BOT (import MAIN ?ALL) (import ENV ?ALL) (import AGENT ?ALL) (import GESTORE_BOT ?ALL) (export ?ALL))

; Questo modulo prova a piazzare un incrociatore solo in verticale partendo dalla cella "bot" conosciuta e 
...
; tutti i valori che servono alle regole di posizionamento per scoprire se è possibile piazzare un incrociatore 
; d'interesse o meno.
;(defrule aggiunta_fatti_per_reg_successive (declare (salience 50))
;    (cella_bot (x ?x) (y ?y) (considerata ?c))
;=>
;    (assert (celle_sopra_a_bot (x_row_sopra_1 (- ?x 1)) (x_row_sopra_2 (- ?x 2))))
;)
...
(defrule posizionamento_incrociatore_in_verticale_conoscendo_bot (declare (salience 26))
...
=>
    ...
    ; (x,y)(dove sappiamo esserci il bot) - (x-1,y) (middle) – (x-3,y)(top) 
    ; e quindi faccio questa assert:
    (assert (piazzamento_incrociatore_verticale (x ?x) (y ?y))) ; per semplicità qui setto solamente la cella da dove partirà l'incrociatore (dal basso)
)
