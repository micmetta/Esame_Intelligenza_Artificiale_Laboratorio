...
;; DA SCRIVERE NEL POWERPOINT:

;; IN realtà un vero limite è questo (Questo problema lo si nota con la mappa2 con 5 fatti iniziali):
;; Nel momento in cui l'agente esegue una unguess e comunque continua a non riuscire
;; a piazzare nulla perchè magari le navi che gli sono rimaste da piazzare non entrano
;; allora NON RIPOSIZIONA LE BANDIERINE CHE AVEVA TOLTO (altrimenti bisognerebbe
;; aggiungere ancora altre regole di controllo nei vari gestori..)
;; Questo accade per l'agente-2 con la mappa-2 con 5 fatti iniziali quando fa la fire su (3,4) e in realtà continua a fare anche
;; altre fire senza ottenere nessun risultato utile per questo l'agente-2 sulla mappa-2 con 5 fatti iniziali fa peggio di quando non
;; aveva nessun fatto iniziale. 


;; ALTRO LIMITE: 
;; nel caso in cui l'agente esegue una fire e questa
;; scopre ad esempio un left, allora se ad esempio lui nella sua struttura
;; sa che può mettere solo un incrociatore e una corazzata e queste due navi
;; in realtà non può metterle perchè a destra di quella cella left può mettere
;; solo il righ allora poichè l'agente assume di aver già posizionato
;; in quel momento tutti i cacciatorpedinieri non sarà in grado di posizionare nessuna nave.
;; L'unica cosa che farà in questo caso è posizionare la bandierina nel left che ha scoperto. 
;; QUESTA COSA ACCADE NELLA FASE 3 DI AGENTE-2 SU MAPPA-CON-5-FATTI-INIZIALI quando scopre che
;; in cella (3,0) c'è un left !! 
;; POSSIBILE MIGLIORAMENTO: Per migliorare questa cosa si potrebbe ad esempio
;; far ragionare l'agente sulle navi che secondo lui gli sono rimaste da piazzare..


;; ALRTRO LIMITE NOTATO PER AGENTE-2 sulla mappa con 5 fatti iniziali:
;; - Con l'informazione iniziale left(9,5) l'agente posiziona il cacciatorpediniere
;;   e non è più in grado di riconoscere che li ci andava una corazzata.. Questo ci fa
;;   capire che l'euristica sulla precedenza da dare alle navi, che viene utilizzata dall'agente-2 
;;   non sempre permette di aumentare lo score finale aumentando il numero di fatti
;;   iniziali conosciuti dall'agente !!